...
extern  "C" {
    ...
    /**
     * The 'file-handle' to a sequence file in hdfs.
     */
    ...
    /**
     * seekLineReader - seek to desiredPos 
     *
     * @param fs The configured filesystem handle.
     * @param lr the LineReader
     * @param desiredPos offset
     * @return Returns 0 on success; <0 on error.
     */
    int seekLineReader(hdfsFS fs, LineReader lr, tOffset desiredPos);

    /**
     * getLineReaderPos - get LineReader pos for next char
     *
     * @param fs The configured filesystem handle.
     * @param lr the LineReader
     * @return Returns pos to next readable char. >=0 on success; <0 on error.
     */
     ...
     * @param f the sequence file handle
     * @param offset the new position
     * @return Returns >0 (offset after sync) on success; -1 on error.
     */
     ...
     * @param fs The configured filesystem handle.
     * @param path the file path
     * @param compressionTypeString compression type: "NONE" - no compression; "RECORD" - record compression;
   ...
   //Followed by is for map/reduce job query

    /**
     * The C reflection of org.apache.org.hadoop.mapred.JobClient .
     */
    typedef void* mapredJC;

    /**
     * MRJobInfo - Information about a map-reduce job's status.
     */
    typedef struct  {
   //   float mapProgress;
   //   float reduceProgress;
     ...
     * JobClientInitialize
     * @param  NULL
     * @return jobect on success else NULL
     */
     mapredJC JobClientInitialize();

    /**
     * JobClientFinalize
     * @param mapredJC object which refer to JobClient
     * @return 0 on success else -1
     */
     ...
     * mapredGetAllJobs
     * @param mapredJC object which refer to JobClient
     * @param numItem indicate the num of returned objects 
     * @return address of the all MRJobInfo objects' array on success else NULL
     */
     ...
     * mapredGetJob
     * @param mapredJC object which refer to JobClient
     * @param jobid the jobid string
     * @return address of the MRJobInfo objects on success else NULL
     */
     MRJobInfo *mapredGetJob(mapredJC jc, const char *jobid);
